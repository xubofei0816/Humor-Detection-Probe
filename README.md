...
Additionally, we performed Mann-Whitney U tests, and rejected all pair-wise null hypotheses, which means significant distributional differences were found between all token groups as expected. We also performed point-biserial tests and found significant correlation between the weights and the binary importance of the tokens.
